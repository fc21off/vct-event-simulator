...
export function createMasters12(teams) {
  const byeTeams = teams.slice(0, 4);
  const swissTeams = teams.slice(4);
  ...
  return {
    ...
    byeTeams,
    stage: 'swiss',
    swiss: createSwissStage(swissTeams),
    bracket: null,
    champion: null
  };
}
...
export function proceedToPlayoffs(tournament) {
  ...
  if (t.stage === 'swiss' && isSwissComplete(t.swiss) && !t.bracket) {
    const swissStandings = getSwissStandings(t.swiss);
    const swissQualified = swissStandings.filter(team => team.wins === 2);
    
    // Bye teams (1-4) + Swiss qualified (5-8)
    const playoffTeams = [...t.byeTeams, ...swissQualified];
    t.bracket = createBracket(playoffTeams);
    t.stage = 'bracket';
  }
  return t;
}
